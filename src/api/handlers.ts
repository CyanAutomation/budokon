...
export function createRestHandlers({ catalog, draw, eventDraw }: { catalog: CatalogService; draw: DrawService; eventDraw?: EventDrawService }) {
  return {
    ...
    drawEvent: ({ body = {} as EventDrawRequest }: ApiRequest<EventDrawRequest> = {}) => { if (!eventDraw) throw new Error("eventDraw service not configured"); return ok(eventDraw.draw(body)); },
