...
export function createRestHandlers({ catalog, draw }) {
  return {
    listJudoka: ({ query = {}, authorizedInternal = false } = {}) => ok(catalog.searchJudoka({ query: query.q, filters: queryFilters(query), exclude: query.exclude === undefined ? [] : Array.isArray(query.exclude) ? query.exclude : [query.exclude], collection: query.collection, includeHidden: query.includeHidden === true, authorizedInternal })),
