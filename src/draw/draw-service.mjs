import { normalizeFilters } from "../domain/catalog-service.mjs";
...
export class DrawService {
  constructor(catalog, random = Math.random) { this.catalog = catalog; this.random = random; }
  draw(input = {}, context = {}) {
    ...
    const random = seed === undefined ? this.random : seededRandom(JSON.stringify({ version: this.catalog.repository.datasetVersion, filters, exclude, count, seed }));
    const remaining = pool.slice(); const judoka = [];
    while (judoka.length < count) judoka.push(remaining.splice(Math.floor(random() * remaining.length), 1)[0]);
