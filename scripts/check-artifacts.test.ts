...
type ArtifactFileMap = Record<string, string | Uint8Array>;

function artifacts(payload = 'canonical\n', overrides = {}): ArtifactFileMap {
  const manifest = {
...
async function fixture(t, files: ArtifactFileMap = artifacts()) {
...
test('artifact check accepts exact compiler output', async (t) => {
  const expectedArtifacts: ArtifactFileMap = artifacts();
